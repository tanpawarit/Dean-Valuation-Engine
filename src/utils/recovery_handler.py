...
class RecoveryHandler:
    ...
    def get_recovery_info(self) -> Mapping[str, object] | None:
    ...
    def recover_state(self, checkpoint_name: str = "current") -> PlanExecuteState | None:
        """
    ...
    def rollback_to_step(self, target_step: int) -> PlanExecuteState | None:
        """
    ...
    def find_last_successful_checkpoint(self) -> tuple[str, PlanExecuteState] | None:
        """
    ...
    def auto_recover(self) -> PlanExecuteState | None:
        """
    ...
    def create_recovery_report(self) -> dict[str, object]:
                ...
                if state:
                    ...
                    if state.get("error_message"):
                        cast(list[str], report["recommendations"]).append(
                            "Current state has errors - consider rollback or auto-recovery"
                        )
                    elif self._can_resume(state):
                        cast(list[str], report["recommendations"]).append("State can be resumed from current position")
                    elif state.get("final_result"):
                        cast(list[str], report["recommendations"]).append("Analysis appears complete")
                    else:
                        cast(list[str], report["recommendations"]).append("State may need manual inspection")
            else:
                cast(list[str], report["recommendations"]).append("No recovery checkpoints available - fresh start required")
